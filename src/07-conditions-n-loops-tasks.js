...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const beg = isStartIncluded ? '[' : '(';
  const end = isEndIncluded ? ']' : ')';
  return `${beg}${Math.min(a, b)}, ${Math.max(a, b)}${end}`;
}
...
function isCreditCardNumber(ccn) {
  let str = `${ccn}`;
  const check = str.at(-1);
  str = str.slice(0, -1);
  let sum = 0;
  const parity = str.length % 2;
  let i = 0;
  while (i < str.length) {
    if (i % 2 === parity) sum += +str[i];
    else if (+str[i] > 4) sum += 2 * (+str[i]) - 9;
    else sum += 2 * (+str[i]);
    i += 1;
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const str1 = pathes[0];
  while (i < str1.length) {
    if (pathes[0][i] !== pathes[pathes.length - 1][i]) { break; }
    res += str1[i];
    i += 1;
  }
